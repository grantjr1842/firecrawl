...
const config: KnipConfig = {
  ...
  ignore: [
    ...
    // W3-ARCH-001: split scrapeURL/index.ts (in flight). The new
    // files are wired in a follow-up commit; ignore until then so
    // the antibot SSRF test matrix (item #6) can land.
    "src/scraper/scrapeURL/fetch.ts",
    "src/scraper/scrapeURL/preflight.ts",
    "src/scraper/scrapeURL/transform.ts",
  ],
